...
class JadwalProvider with ChangeNotifier {
  ...
  Future<bool> updateJadwalStatus(
      int jadwalId, bool newStatus, String guruIdPemilik,
      {String? idMurid} // 🔥 TAMBAHAN: Opsional ID Murid
      ) async {
    bool sukses = false;
    try {
  ...
  // --- (READ) JADWAL SAYA (Untuk Murid) ---
  List<Map<String, dynamic>> _jadwalSaya = [];
  bool _isLoadingJadwalSaya = false;

  List<Map<String, dynamic>> get jadwalSaya => _jadwalSaya;
  bool get isLoadingJadwalSaya => _isLoadingJadwalSaya;

  Future<void> fetchJadwalSaya(String muridId) async {
    _isLoadingJadwalSaya = true;
    notifyListeners();

    try {
      // Kita perlu buat method di Repo dulu, tapi untuk sekarang kita siapkan di Provider
      final data = await _jadwalRepository.getJadwalSaya(muridId);
      _jadwalSaya = data;
    } catch (e) {
      print("Error fetchJadwalSaya: $e");
      _jadwalSaya = [];
    }

    _isLoadingJadwalSaya = false;
    notifyListeners();
  }
  ...
  Future<bool> deleteJadwal(int jadwalId, String guruIdPemilik) async {
    ...
    try {
      ...
      if (response['success'] == true) {
        ...
        _jadwalMilikGuru
            .removeWhere((jadwal) => jadwal['jadwal_id'] == jadwalId);
        sukses = true;
      }
    } catch (e) {
  ...
  List<JadwalLesModel> getJadwalForGuru(String email) {
    ...
    return _jadwalBeranda
        .where(
            (jadwal) => jadwal.emailGuru.toLowerCase() == email.toLowerCase())
        .toList();
  }
}
