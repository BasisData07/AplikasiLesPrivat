...
router.post('/update-password-direct', async (req, res) => {
  ...
  try {
    ...
    // 1. Cek di GURU
    // PERBAIKAN: Gunakan 'await db.execute' (Array Destructuring)
...
router.post('/delete-account', async (req, res) => {
  ...
  try {
    ...
    if (role === 'guru') {
      // --- Logika Hapus Guru ---
      // 1. Ambil data user dulu untuk cek password (pakai await db.execute)
      const [results] = await db.execute('SELECT * FROM akun_guru WHERE guru_id = ?', [userId]);
      ...
      // --- Logika Hapus Murid ---
      const [results] = await db.execute('SELECT * FROM akun_pengguna WHERE pengguna_id = ?', [userId]);
...
router.get('/users', (req, res) => {
  ...
  // Tidak perlu try-catch di sini karena error ditangani di callback Model
  getAll((err, users) => {
