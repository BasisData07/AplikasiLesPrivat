...
class _GuruBerandaPageState extends State<GuruBerandaPage> {
  ...
  Future<void> _fetchUlasan() async {
    ...
    try {
      ...
      final ulasan =
          await UlasanService.getUlasanByGuruId(widget.user.id.toString());
      if (mounted) {
        setState(() {
          _ulasanList = ulasan;
  ...
  void _showReplyDialog(int ulasanId, String namaMurid, String komentarMurid) {
    ...
    showDialog(
      ...
      builder: (dialogContext) {
        // Gunakan dialogContext untuk pop dialog
        return AlertDialog(
                ...
                if (mounted) {
                  if (response['success'] == true) {
                    scaffoldMessenger.showSnackBar(
                        // Menggunakan referensi yang disimpan (lebih aman)
                        SnackBar(
                    ...
                    scaffoldMessenger.showSnackBar(
                      ...
                      SnackBar(
                          content: Text(
                              response['message'] ?? "Gagal mengirim balasan."),
  ...
  // --- FUNGSI 2: HANDLE KLIK CHECKLIST (ASSIGN / UN-ASSIGN) ---
  Future<void> _handleJadwalAction(
      int jadwalId, bool currentlyBooked, String? currentMuridId) async {
    // A. JIKA SUDAH DIPESAN -> KONFIRMASI PEMBATALAN
    if (currentlyBooked) {
      bool? yakinBatal = await showDialog<bool>(
        context: context,
        builder: (BuildContext ctx) {
          return AlertDialog(
            title: const Text('Batalkan Pesanan?'),
            content: const Text(
              'Jadwal ini sudah ditandai dipesan. Apakah Anda ingin membatalkannya (membuatnya tersedia lagi)?',
            ),
            actions: [
              TextButton(
                  onPressed: () => Navigator.of(ctx).pop(false),
      ...
      if (yakinBatal != true || !mounted) return;

      // Eksekusi Un-book (Set is_booked = false)
      _processUpdateStatus(jadwalId, false, null);
    } else {
      // B. JIKA BELUM DIPESAN -> PILIH MURID UNTUK DI-ASSIGN
      // Ambil daftar murid dari Chat Service (user yg pernah chat dengan guru)
      // Idealnya ada tabel 'enrollment', tapi kita pakai riwayat chat sbg 'My Students'

      showDialog(
        context: context,
        builder: (dialogContext) {
          return StreamBuilder<List<String>>(
            stream: ChatService().getPeers(
                widget.user.id.toString()), // Gunakan getPeers (Stream)
            builder: (context, snapshot) {
              ...
              if (studentIds.isEmpty) {
                return AlertDialog(
                  ...
                  content: const Text(
                      "Anda belum memiliki riwayat chat dengan murid manapun. Tidak ada murid yang bisa dipilih."),
                  actions: [
                    TextButton(
              ...
              return AlertDialog(
                title: const Text("Assign ke Murid"),
                content: SizedBox(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (ctx, idx) {
                      final sId = studentIds[idx];
                      return FutureBuilder<String>(
                          future: ChatService().getUserName(sId),
                          builder: (context, nameSnap) {
                            final sName = nameSnap.data ?? "Murid $sId";
                            return ListTile(
                              leading:
                                  const CircleAvatar(child: Icon(Icons.person)),
                              title: Text(sName), // 🔥 Tampilkan Nama
                              onTap: () {
                                Navigator.pop(
                                    dialogContext); // Tutup dialog pilih murid
                                // Konfirmasi akhir
                                _showConfirmAssign(jadwalId, sId, sName);
                              },
                            );
                          });
                    },
  ...
  void _showConfirmAssign(int jadwalId, String muridId, String muridName) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text("Konfirmasi"),
        content: Text("Tandai jadwal ini dipesan oleh $muridName?"),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.pop(ctx);
              _processUpdateStatus(jadwalId, true, muridId);
            },
  ...
  Future<void> _processUpdateStatus(
      int jadwalId, bool newStatus, String? muridId) async {
    if (!mounted) return;

    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Mengupdate status...')),
    );

    final jadwalProvider = Provider.of<JadwalProvider>(context, listen: false);
    final sukses = await jadwalProvider.updateJadwalStatus(
        jadwalId, newStatus, widget.user.id.toString(),
        idMurid: muridId);
          ...
          itemBuilder: (context, index) {
            ...
            return Card(
              ...
              child: ListTile(
                ...
                title: Text(ulasan.komentarMurid,
                    maxLines: 1, overflow: TextOverflow.ellipsis),
                subtitle: Text("Dari ${ulasan.namaMurid} (${ulasan.rating}⭐)"),
                trailing: ElevatedButton(
                  onPressed: () => _showReplyDialog(
                      ulasan.ulasanId, ulasan.namaMurid, ulasan.komentarMurid),
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final String namaMapel =
                        slot['nama_mapel'] ?? 'Tanpa Mapel';
                    final String hari = slot['hari'] ?? 'Tanpa Hari';
                    final String jamMulaiFull = slot['jam_mulai'] ?? '00:00';
                    final String jamSelesaiFull =
                        slot['jam_selesai'] ?? '00:00';
                    final String jamMulai = jamMulaiFull.length >= 5
                        ? jamMulaiFull.substring(0, 5)
                        : jamMulaiFull;
                    final String jamSelesai = jamSelesaiFull.length >= 5
                        ? jamSelesaiFull.substring(0, 5)
                        : jamSelesaiFull;
                    final int jadwalId = slot['jadwal_id'];

                    final bool isBooked =
                        (slot['is_booked'] == 1 || slot['is_booked'] == true);
                    final String statusText =
                        isBooked ? 'Dipesan Murid' : 'Tersedia';
                          ...
                          children: [
                            ...
                            IconButton(
                              icon: Icon(
                                ...
                                color:
                                    isBooked ? Colors.orange : Colors.grey[600],
                              ),
                              onPressed: () => _handleJadwalAction(jadwalId,
                                  isBooked, slot['pengguna_id']?.toString()),
                              tooltip: isBooked
                                  ? 'Batalkan Pesanan'
                                  : 'Assign ke Murid',
                            ),
        ...
        builder: (context, snapshot) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              // Tampilan Item Chat
              return FutureBuilder<String>(
                  future: ChatService().getUserName(peerId),
                  builder: (context, snapshotName) {
                    final name = snapshotName.data ?? "Murid $peerId";
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        title: Text(name), // 🔥 Tampilkan Nama
