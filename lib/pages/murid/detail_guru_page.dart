...
class _DetailGuruPageState extends State<DetailGuruPage> {
  ...
  Future<void> _fetchGuruDetail() async {
    ...
    if (mounted) {
      setState(() {
        _detailGuruData =
            data; // data from GuruService.getDetailGuru already returns result['data'] based on previous view? No, wait.
        // Step 4 view_file:
        // 75: final data = await GuruService.getDetailGuru(guruId);
        // 79: _detailGuruData = data?['data'];

        // Wait, review GuruService in Step 34.
        // static Future<Map<String, dynamic>?> getDetailGuru(String id) async {
        //   final result = await ApiService.get('profile/detail/$id');
        //   if (result['success'] == true) {
        //     return result['data']; // Returns 'data' object directly!
        //   }
        //   return null;
        // }

        // In detail_guru_page.dart, line 79: `_detailGuruData = data?['data'];`
        // If `data` IS ALREADY `result['data']`, then `data['data']` might be NULL if the structure isn't nested twice!
        // THIS MIGHT BE THE BUG.

        _detailGuruData = data;
        print("DEBUG_GURU_DATA: $_detailGuruData");
        _isLoading = false;
      });
    }
  }
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                if (komentar.isEmpty || ratingValue < 1) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                        content: Text("Rating dan komentar wajib diisi.")),
                  );
                  return;
                }
                ...
                final response = await UlasanService.submitUlasan(
                  ...
                  penggunaId:
                      widget.user.id.toString(), // ID Murid yang sedang login
                  rating: ratingValue,
                  komentar: komentar,
                );
                ...
                if (mounted) {
                  ...
                  if (response['success'] == true) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                          content: Text(
                              response['message'] ?? "Gagal mengirim ulasan.")),
                    );
                  }
                }
              },
  ...
  Future<void> openWhatsApp(String number) async {
    final String text = Uri.encodeComponent(
        "Halo kak, saya ingin bertanya tentang les privat.");
  ...
  Widget build(BuildContext context) {
    ...
    final String displayName =
        _detailGuruData?['nama_lengkap'] ?? widget.guru.nama;
    final String displayBio =
        _detailGuruData?['bio_deskripsi'] ?? widget.guru.deskripsi;
    final String displayLevel =
        _detailGuruData?['list_jenjang'] ?? widget.guru.kategori_jenjang;
    final String displayLokasi =
        _detailGuruData?['domisili'] ?? widget.guru.kota;
    final String displayTelpon =
        _detailGuruData?['no_telpon'] ?? widget.guru.noTelepon;
    //final String isplayWatsApp = _detailGuruData?['no_whatsapp'] ?? widget.guru.noWhatsApp;

    final String displayPengalamanRaw =
        _detailGuruData?['pengalaman_tahun']?.toString() ??
            widget.guru.pengalaman.replaceAll(' Thn', '');
    final String displayPengalaman = displayPengalamanRaw.split(' ').first;

    final dynamic rawHarga =
        _detailGuruData?['harga_per_jam'] ?? widget.guru.harga;
    final String displayHarga = _formatCurrency(rawHarga);
    
    final String displayMapelString =
        _detailGuruData?['list_mapel'] ?? widget.guru.mapel;
    ...
    final String? displayFoto =
        _detailGuruData?['foto_profil_guru'] ?? widget.guru.foto;
    // -----------------------------------------------------------------
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildStatsCard(cardColor, textColor, subTextColor,
                      displayPengalaman, displayHarga, displayRating),
                  const SizedBox(height: 24),
                  ...
                  Container(
                    ...
                    child: Text(
                        displayBio.isEmpty
                            ? "Guru belum mengisi deskripsi diri."
                            : displayBio,
                        style: TextStyle(
                    ...
                    child: Column(
                      children: [
                        _buildInfoRow(
                            ...
                            "Tingkat",
                            displayLevel.split(', ').first,
                            subTextColor,
                            textColor),
                        const Divider(height: 1),
                        _buildInfoRow(Icons.location_city_outlined, "Domisili",
                            displayLokasi, subTextColor, textColor),
                        const Divider(height: 1),
                        Padding(
                          padding: const EdgeInsets.symmetric(
                            ...
                            children: [
                              ...
                              Row(
                                ...
                                children: [
                                  ...
                                  InkWell(
                                    onTap: () =>
                                        openWhatsApp(widget.guru.noTelepon),
                                    child: Container(
                  ...
                  Wrap(
                      spacing: 8,
                      runSpacing: 8,
                      children: displayMapelString
                          .split(', ')
                          .where((mapel) => mapel.trim().isNotEmpty)
                          .map((mapel) {
                        return Chip(
                            label: Text(mapel,
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        const Spacer(),
                        Text(ulasan.namaMurid,
                            style: TextStyle(color: subTextColor)),
                      ],
                    ),
                    const SizedBox(height: 8),
                    Text(ulasan.komentarMurid,
                        style: TextStyle(color: textColor)),
                    if (ulasan.balasanGuru != null &&
                        ulasan.balasanGuru!.isNotEmpty)
                      Padding(
                        padding: const EdgeInsets.only(top: 10, left: 16),
                        child: Text("Balasan Guru: ${ulasan.balasanGuru}",
                ...
                subtitle: Text(slot.jam,
                    style: TextStyle(
                        color: widget.isDarkMode
                            ...
                            : Colors.grey[600])),
                trailing: Chip(
                  label: Text(slot.isBooked ? "Dipesan" : "Tersedia",
                      style: const TextStyle(
                          color: Colors.white,
                          fontSize: 12,
                          fontWeight: FontWeight.bold)),
  ...
  Widget _buildStatsCard(Color? cardColor, Color textColor, Color? subTextColor,
      String peng, String harga, double rating) {
    return Card(
      color: cardColor,
      elevation: 0,
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            ...
            _buildStatItem(
                Icons.work_outline, peng, "Tahun", textColor, subTextColor),
            _buildStatItem(Icons.price_change_outlined, harga, "/ Jam",
  ...
  Widget _buildRatingStatItem(
      double rating, Color textColor, Color? subTextColor) {
    ...
    return Column(
      children: [
        Icon(rating > 0 ? Icons.star : Icons.star_border,
            color: rating > 0 ? getRatingColor(rating) : Colors.grey, size: 28),
        const SizedBox(height: 8),
        Text(displayRating,
            style: TextStyle(
                fontSize: 18, fontWeight: FontWeight.bold, color: textColor)),
  ...
  Widget _buildTappableInfoRow(
      IconData icon, String title, Color? subTextColor, VoidCallback onTap,
      {double verticalPadding = 12.0}) {
    return InkWell(
      onTap: onTap,
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          ElevatedButton(
            onPressed: () {
              bool isAlreadyFavorite = widget.favoriteTeachers
                  .any((favGuru) => favGuru.email == widget.guru.email);
