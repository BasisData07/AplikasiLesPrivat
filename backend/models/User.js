...
class User {
  ...
  static async _getMapelIdPromise(mapelName) {
    ...
    // Sesuaikan nama kolom tabel mapel (biasanya nama_mapel atau subject)
    // Asumsi di sini nama kolomnya 'nama_mapel' karena di kode lamamu begitu.
    ...
    bcrypt.hash(password, 10, async (err, hash) => {
      ...
      try {
        if (role === 'guru') {
          ...
          // 2. Insert Guru
          // KOREKSI: Gunakan 'name' dan 'subject' sesuai database kamu
  ...
  static async login(email, password, callback) {
    try {
      // 1. Cek Guru
      // KOREKSI: Query diganti jadi 'name' dan 'subject'
      ...
      // 2. Cek Murid
      // KOREKSI: Query diganti jadi 'name'
  ...
  static async getAll(callback) {
    try {
      // KOREKSI: Menggunakan 'name' dan 'subject'
