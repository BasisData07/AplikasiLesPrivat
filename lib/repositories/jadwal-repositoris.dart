...
class JadwalRepository {
  ...
  Future<List<GuruMapelModel>> getMapelMilikGuru(String guruId) async {
    try {
      ...
      if (response['success'] == true) {
        ...
        throw Exception(
            response['message'] ?? 'Gagal memuat daftar mapel guru.');
      }
    } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> getJadwalMilikGuru(String guruId) async {
    try {
      ...
      if (response['success'] == true) {
        ...
        throw Exception(
            response['message'] ?? 'Gagal memuat jadwal milik guru.');
      }
    } catch (e) {
  ...
  // -----------------------------------------------------------------
  // === (UPDATE STATUS) Untuk fitur Checklist & Assign ===
  // -----------------------------------------------------------------
  Future<Map<String, dynamic>> updateJadwalStatus({
    required int jadwalId,
    required bool newStatus,
    required String guruIdPemilik,
    String? idMurid, // 🔥 TAMBAHAN
  }) async {
    ...
    // 🔥 Jika ada idMurid, kirim juga
    if (idMurid != null) {
      data['pengguna_id'] = idMurid;
    }
  ...
  // -----------------------------------------------------------------
  // === (READ) Jadwal Saya (Murid) ===
  // -----------------------------------------------------------------
  Future<List<Map<String, dynamic>>> getJadwalSaya(String muridId) async {
    try {
      final response = await ApiService.get('jadwal/murid/$muridId');
      ...
      if (response['success'] == true) {
        ...
        throw Exception(response['message'] ?? 'Gagal memuat jadwal saya.');
      }
    } catch (e) {
      print("Error getJadwalSaya: $e");
      throw Exception('Gagal memuat jadwal saya: $e');
    }
  }
